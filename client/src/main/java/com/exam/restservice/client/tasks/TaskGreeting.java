...
public class TaskGreeting {
    ...
    protected static boolean runningPart(RequestSender<String> req, Long id, String threadName) {
        ...
        if (response.getStatusCode() == HttpStatus.OK) {
            ...
            Wrapper<Greeting> responseObject = gson.fromJson(res, new TypeToken<Wrapper<Greeting>>() {
            }.getType());
            if (responseObject != null) {
                Wrapper<String> reqMsg = gson.fromJson(
                        gson.toJson(
                                responseObject.getTechInfo().get(CommonNames.ParamsNames.PARAM_REQUEST)
                        ), new TypeToken<Wrapper<String>>() {
                        }.getType()
                );

                String thread = responseObject.getTechInfo().getOrDefault(CommonNames.ParamsNames.PARAM_THREAD_NAME, "QQL").toString();
